...
import { 
  Plus, Wallet, ArrowUpRight, ArrowDownLeft, AlertCircle, X, CheckCircle2, Crown 
} from 'lucide-react';
...
export default function Debts() {
  ...
  const handleLend = async (e: React.FormEvent) => {
      ...
      try {
          ...
          setShowForm(false);
          setFormData({ borrower_id: '', new_borrower_name: '', amount: '', date: '', due_date: '', reason: '' });
          router.invalidate();
      } catch (e: any) { 
          console.error("Lending Error:", e);
          alert("Failed to record loan: " + (e.response?.data?.detail || e.message)); 
      }
  };
    ...
    <div className="space-y-8 animate-fade-in pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
            <div>
                ...
                <p className="text-stone-500 dark:text-slate-400">Track money you've lent and outstanding repayments.</p>
            </div>
            <button 
                onClick={() => setShowForm(true)} 
                className="bg-stone-900 dark:bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:bg-stone-800 dark:hover:bg-indigo-500 transition shadow-lg"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-stone-50 dark:border-slate-800 shadow-sm transition-colors">
                <div className="flex items-center gap-3 mb-2">
                    <div className="p-2 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg"><ArrowUpRight size={20}/></div>
                    <span className="text-xs font-bold uppercase text-stone-400 dark:text-slate-500">Total Lent</span>
                </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-stone-50 dark:border-slate-800 shadow-sm transition-colors">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-xs font-bold uppercase text-stone-400 dark:text-slate-500">Total Repaid</span>
                </div>
                <p className="text-3xl font-black text-stone-800 dark:text-white">₹{stats?.total_repaid?.toLocaleString() || 0}</p>
            </div>

            <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-stone-50 dark:border-slate-800 shadow-sm relative overflow-hidden transition-colors">
            ...
            <div className="bg-indigo-600 dark:bg-indigo-900/40 p-6 rounded-[2rem] text-white shadow-lg shadow-indigo-200 dark:shadow-none flex flex-col justify-between">
                <div>
                   <div className="flex items-center gap-2 mb-3 opacity-90">
                       ...
                       <span className="text-xs font-bold uppercase tracking-wider">Top Outstanding</span>
                   </div>
        ...
        {showForm && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div className="relative bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2rem] shadow-2xl p-6 animate-in zoom-in-95 border border-stone-100 dark:border-slate-800">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="font-bold text-xl text-stone-800 dark:text-white">Record New Loan</h3>
                        <button onClick={() => setShowForm(false)}><X className="text-stone-400 hover:text-stone-600 dark:text-slate-500 dark:hover:text-slate-300"/></button>
                    </div>
                    ...
                    <form onSubmit={handleLend} className="space-y-4">
                        ...
                        <div className="bg-stone-50 dark:bg-slate-800 p-4 rounded-xl space-y-3 border border-stone-100 dark:border-slate-700">
                            <label className="text-xs font-bold uppercase text-stone-400 dark:text-slate-500">Who are you lending to?</label>
                            <select 
                                className="w-full p-3 rounded-lg bg-white dark:bg-slate-900 text-stone-800 dark:text-white border border-stone-200 dark:border-slate-700 outline-none focus:ring-2 focus:ring-indigo-500"
                            ...
                            {!formData.borrower_id && (
                                <input 
                                    ...
                                    className="w-full p-3 rounded-lg bg-white dark:bg-slate-900 text-stone-800 dark:text-white border border-stone-200 dark:border-slate-700 outline-none focus:ring-2 focus:ring-indigo-500"
                                    value={formData.new_borrower_name}
                                    onChange={e => setFormData({...formData, new_borrower_name: e.target.value})}
                                />
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" required className="w-full p-3 mt-1 rounded-xl bg-stone-50 dark:bg-slate-800 text-stone-800 dark:text-white font-bold outline-none border border-transparent focus:border-indigo-500" placeholder="₹" value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} />
                            </div>
                            <div>
                                <label className="text-xs font-bold uppercase text-stone-400 dark:text-slate-500">Date</label>
                                <input type="date" required className="w-full p-3 mt-1 rounded-xl bg-stone-50 dark:bg-slate-800 text-stone-800 dark:text-white font-bold outline-none border border-transparent focus:border-indigo-500" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />
                            </div>
                        </div>
                        
                        <div>
                             <label className="text-xs font-bold uppercase text-stone-400 dark:text-slate-500">Due Date (Optional)</label>
                             <input type="date" className="w-full p-3 mt-1 rounded-xl bg-stone-50 dark:bg-slate-800 text-stone-500 dark:text-slate-400 font-bold outline-none border border-transparent focus:border-indigo-500" value={formData.due_date} onChange={e => setFormData({...formData, due_date: e.target.value})} />
                        </div>

                        <div>
                            <label className="text-xs font-bold uppercase text-stone-400 dark:text-slate-500">Reason / Note</label>
                            <input required className="w-full p-3 mt-1 rounded-xl bg-stone-50 dark:bg-slate-800 text-stone-800 dark:text-white font-bold outline-none border border-transparent focus:border-indigo-500" placeholder="e.g. Dinner bill split" value={formData.reason} onChange={e => setFormData({...formData, reason: e.target.value})} />
                        </div>
                ...
                <div key={b.id} onClick={() => openLedger(b)} className="group bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-stone-50 dark:border-slate-800 shadow-sm hover:shadow-md transition cursor-pointer relative overflow-hidden">
                     ...
                     <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-stone-100 dark:bg-slate-800 rounded-full flex items-center justify-center text-stone-500 dark:text-slate-400 font-bold text-lg border border-stone-200 dark:border-slate-700">
                                {b.current_balance <= 0 ? (
                            ...
                            <div>
                                ...
                                <p className="text-xs text-stone-400 dark:text-slate-500">Last active: {new Date(b.last_activity).toLocaleDateString()}</p>
                            </div>
                        </div>
                     ...
                     <div className="mt-4 h-2 w-full bg-stone-100 dark:bg-slate-800 rounded-full overflow-hidden">
                        <div 
                            ...
                            style={{ width: `${(b.total_repaid / (b.total_lent || 1)) * 100}%` }} 
                        />
                     </div>
                </div>
            ))}
...
function BorrowerLedger({ borrower, onClose }: { borrower: Borrower, onClose: () => void }) {
    const [history, setHistory] = useState<{debts: any[], repayments: any[]} | null>(null);
    ...
    const handleRepay = async (e: React.FormEvent) => {
        e.preventDefault();
        if(!repayData.debt_id) return alert("Please select which loan is being repaid");
        
        try {
            await axios.post(`https://finance-tracker-q60v.onrender.com/debts/repay`, {
             ...
             <div className="relative bg-white dark:bg-slate-900 w-full md:max-w-2xl h-[90vh] md:h-auto md:max-h-[85vh] rounded-t-[2.5rem] md:rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 border border-stone-200 dark:border-slate-800">
                 ...
                 <div className="p-6 md:p-8 bg-stone-50 dark:bg-slate-800 border-b border-stone-100 dark:border-slate-700 flex justify-between items-start shrink-0">
                     <div>
                         ...
                         <p className="text-stone-500 dark:text-slate-400 text-sm mt-1">Transaction History</p>
                     </div>
                 ...
                 <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
                     ...
                     {/* 1. Pending Loans Section */}
                     <div>
                         <h4 className="text-xs font-bold uppercase text-stone-400 dark:text-slate-500 mb-4 tracking-wider">Active Loans</h4>
                         <div className="space-y-3">
                             {history?.debts.filter(d => d.status !== 'Paid').map((debt: any) => (
                                 <div key={debt.id} className="bg-white dark:bg-slate-950 border border-stone-100 dark:border-slate-800 p-4 rounded-2xl flex justify-between items-center shadow-sm">
                                     <div>
                                         <p className="font-bold text-stone-800 dark:text-white">{debt.reason}</p>
                                         <p className="text-xs text-stone-500 dark:text-slate-400">{new Date(debt.date).toLocaleDateString()} • Due {debt.due_date || 'N/A'}</p>
                                     </div>
                                     <div className="text-right">
                                         <p className="font-bold text-stone-800 dark:text-white">₹{debt.amount}</p>
                                         <p className="text-xs font-bold text-rose-500">Left: ₹{debt.amount - debt.amount_repaid}</p>
                                     </div>
                                 </div>
                             ))}
                             {history?.debts.filter(d => d.status !== 'Paid').length === 0 && (
                                 <p className="text-sm text-stone-400 italic">No active loans.</p>
                             )}
                         </div>
                     </div>
                     ...
                     {/* 2. Repayment History */}
                     <div>
                         <h4 className="text-xs font-bold uppercase text-stone-400 dark:text-slate-500 mb-4 tracking-wider mt-6">Recent Activity</h4>
                         <div className="relative border-l-2 border-stone-100 dark:border-slate-800 ml-3 space-y-6 pb-4">
                             {[...history?.debts || [], ...history?.repayments || []]
                                .sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime())
                                .map((item: any, idx: number) => {
                                    const isRepayment = item.mode !== undefined; 
                                    return (
                                        <div 
                                          key={`${isRepayment ? 'r':'d'}-${item.id}`} 
                                          className="ml-6 relative animate-in slide-in-from-left-2 duration-500 fill-mode-backwards"
                                            ...
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <p className={`text-sm font-bold ${isRepayment ? 'text-emerald-600 dark:text-emerald-400' : 'text-stone-800 dark:text-white'}`}>
                                                        {isRepayment ? `Repayment via ${item.mode}` : `Lent: ${item.reason}`}
                                                    </p>
                         ...
                         <form onSubmit={handleRepay} className="bg-stone-50 dark:bg-slate-800 p-4 rounded-2xl animate-in slide-in-from-bottom-4 border border-stone-100 dark:border-slate-700">
                             ...
                             <div className="grid grid-cols-2 gap-3 mb-3">
                                <select required className="col-span-2 p-3 rounded-xl bg-white dark:bg-slate-900 text-stone-800 dark:text-white border-none outline-none text-sm" value={repayData.debt_id} onChange={e => setRepayData({...repayData, debt_id: e.target.value})}>
                                    ...
                                    {history?.debts.filter(d => d.status !== 'Paid').map(d => (
                                        <option key={d.id} value={d.id}>{d.reason} (Bal: ₹{d.amount - d.amount_repaid})</option>
                                    ))}
                                </select>
