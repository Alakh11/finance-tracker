...
export default function BudgetList({ budgets }: Props) {
  return (
    <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-stone-50 shadow-sm space-y-8">
        <div className="flex items-center justify-between">
            <h3 className="font-bold text-stone-700 text-lg">Category Breakdown</h3>
                ...
                <div key={b.category_id} className="group relative">
                    ...
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                            <span className="text-2xl bg-stone-50 w-10 h-10 flex items-center justify-center rounded-xl shadow-sm border border-stone-100">
                                {b.icon}
                            </span>
                            <div>
                                <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3">
                                    <p className="font-bold text-stone-800">{b.name}</p>
                                    <span className="text-[10px] font-bold text-stone-500 bg-stone-100 px-2 py-0.5 rounded-md w-fit">
                        ...
                        {b.is_over ? (
                            <div className="flex items-center gap-1.5 text-rose-500 bg-rose-50 px-3 py-1.5 rounded-xl text-xs font-bold animate-pulse">
                                <AlertTriangle className="w-3.5 h-3.5" />
                                <span className="hidden md:inline">Over Budget</span>
                                <span className="md:hidden">Over</span>
                            </div>
                        ) : (
                            <div className="flex items-center gap-1.5 text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-xl text-xs font-bold">
                    ...
                    {/* Progress Bar */}
                    <div className="h-3 w-full bg-stone-100 rounded-full overflow-hidden mb-2 border border-stone-50">
                        <div 
                    ...
                    <div className="flex justify-between items-center text-xs font-medium">
                        <span className="text-stone-500">
                            Spent: <span className="text-stone-900 font-bold">₹{b.spent.toLocaleString()}</span>
                        </span>
